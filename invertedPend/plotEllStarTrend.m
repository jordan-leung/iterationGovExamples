...
%% Run loop to plot the peak ell_k

NVec_peak = [5 10 15 20 25 30 35 40];
NSample = length(NVec_peak);
ellPeak_cs = zeros(NSample,1);
ellPeak_ws = zeros(NSample,1);
for iOuter = 1:NSample
    loadstr = ['./Data/output_ig_N',num2str(NVec_peak(iOuter)),'_ic1_ws0'];
    load(loadstr);
    ellPeak_cs(iOuter) = max(output.ellHist);

    % loadstr = ['output_ig_N',num2str(NVec_peak(iOuter)),'_ic1_ws1'];
    % load(loadstr);
    % ellPeak_ws(iOuter) = max(output.ellHist);
end

figure(1)
semilogy(NVec,ellStar_cs,'b')
hold on; box on; grid on;
semilogy(NVec_peak,ellPeak_cs,'b-.')

    % Save the data (will plot in other file)
    save('./Data/ellStarTrend','NVec','ellStar_cs','NVec_peak','ellPeak_cs');
